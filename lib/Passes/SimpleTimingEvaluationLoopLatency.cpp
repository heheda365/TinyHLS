...
bool SimpleTimingEvaluation::isInLoop(BasicBlock *BB) {
...
*/
Loop *SimpleTimingEvaluation::getOuterLoopOfBlock(BasicBlock *B) {
...
void SimpleTimingEvaluation::getLoopBlockMap(Function *F) {
...
*/
Loop *SimpleTimingEvaluation::getInnerUnevaluatedLoop(Loop *outerL) {
  int dep = 0;
    ...
    *Evaluating_log << "--------- checking sub-loop: " << tmp_Loop->getName()
    ...
    if (tmp_inner_Sub_Loop) {
      if (tmp_inner_Sub_Loop->getLoopDepth() > dep &&
        ...
        tmp_inner_Loop = tmp_inner_Sub_Loop; //  the sub-sub-...-loop could be
                                             //  the most inner loop
      ...
      if (tmp_Loop->getLoopDepth() > dep &&
        ...
        dep = tmp_Loop->getLoopDepth();
        tmp_inner_Loop = tmp_Loop; //  no the sub-sub-...-loop could be the most
                                   //  inner loop but current sub-loop could be
        *Evaluating_log << "--------- update target sub-loop to Loop: "
                        << tmp_inner_Loop->getName() << "\n";
      }
    }
  ...
  if (tmp_inner_Loop ==
      NULL) // all sub-loops are evaluated, check the loop itself.
  {
...
/*
    ...
    (4) mark the blocks in loop with the loop latency, so later processing can
   regard this loop as an integration
*/
double SimpleTimingEvaluation::getOuterLoopLatency(Loop *outerL) {
  *Evaluating_log << "\n Evaluating Outer Loop Latency for Loop "
                  << outerL->getName() << ":\n";
  if (LoopLatency.find(outerL) != LoopLatency.end()) {
    *Evaluating_log << "Done evaluation outer Loop Latency for Loop "
                    << outerL->getName() << " and its latency is "
                    << LoopLatency[outerL] << " cycles.\n\n\n";
  ...
  while (cur_Loop != NULL) {
    *Evaluating_log << "-- Handling the inner Loop " << cur_Loop->getName()
                    << ":\n";
    BasicBlock *tmp_LoopHeader =
        cur_Loop->getHeader(); // get the header of the loop
    *Evaluating_log << "---- its header: " << tmp_LoopHeader->getName()
                    << ":\n";
    SmallVector<BasicBlock *, 8> tmp_ExitingBlocks;
    cur_Loop->getExitingBlocks(
        tmp_ExitingBlocks); // get the exiting blocks of the loop
    if (tmp_ExitingBlocks.size() != 1) {
      assert(tmp_ExitingBlocks.size() > 0);
      print_warning(
          "the loop could be better to have only one exiting block for the "
    ...
    *Evaluating_log << "\n";
    ...
    tmp_BlockCriticalPath_inLoop
        .clear(); // record the block level critical path in the loop
    tmp_SubLoop_CriticalPath.clear(); // record the critical path to the end of
                                      // sub-loops in the loop

    BlockVisited.clear();
    LoopLatencyEvaluation_traversFromHeaderToExitingBlocks(0, cur_Loop,
                                                           tmp_LoopHeader);
    ...
    // COMMENT because preheader is not in the loop enity and if the prehearder
    // is calculated, it is actually duplicated calculation. but just need to
    // add one cycle, as it seems that in VivadoHLS, Loops are regarded as
    // function and the "call" will take one cycle if
    // (cur_Loop->getLoopPreheader())
    //     tmp_total_latency +=
    //     BlockLatencyEvaluation(cur_Loop->getLoopPreheader());
    tmp_total_latency += 1;

    // (4) mark the blocks in loop with the loop latency, so later processing
    // can regard this loop as an integration
    BlockVisited.clear();
    MarkBlock_traversFromHeaderToExitingBlocks(tmp_total_latency, cur_Loop,
                                               tmp_LoopHeader);
    ...
    *Evaluating_log << "Done evaluation Loop Latency for Loop "
                    << cur_Loop->getName() << " and its latency is "
                    << tmp_total_latency << " cycles.\n\n\n";

    // (1) iteratively handle the most inner loop
    cur_Loop = getInnerUnevaluatedLoop(outerL);
  }
  outerL_latency = tmp_total_latency; // finally, we will get the latency of
                                      // outer loop in the last iteration
  *Evaluating_log << "Done evaluation outer Loop Latency for Loop "
                  << outerL->getName() << " and its latency is "
                  << outerL_latency << " cycles.\n\n\n";
  assert(outerL_latency > -0.5 &&
         "The latency for a loop should be not be negative");
...
/*
    ...
    (3a) -- If it is a block in sub-loops, regard the loop as intergration and
   update the critical path if necessary (max(ori_CP, lastStateCP +
   LoopLatency)).
         -- find the successors of the loop by checking its exiting blocks'
   successors and continue the DFS (3b) -- If it is a block out of sub-loops,
   evaluate the block latency and update the critical path if necessary
   (max(ori_CP, lastStateCP + BlockLatency)).
         -- find the successors of the block and continue the DFS
    (4) Release the block from visited flag, as a step of typical DFS

*/
void SimpleTimingEvaluation::
    LoopLatencyEvaluation_traversFromHeaderToExitingBlocks(
  ...
  *Evaluating_log << "---- traverser arrive Block: " << curBlock->getName()
  ...
  if (Block2EvaluatedLoop.find(curBlock) != Block2EvaluatedLoop.end()) {
    // (3a) -- If it is a block in sub-loops, regard the loop as intergration
    // and update the critical path if necessary (max(ori_CP, lastStateCP +
    // LoopLatency)).
    ...
    *Evaluating_log << " LoopLatency =  " << LoopLatency[tmp_SubLoop] << " ";
    double try_critical_path =
        tmp_critical_path +
        LoopLatency[tmp_SubLoop]; // first, get the critical path to the end of
                                  // sub-loop
    *Evaluating_log << " NewCP =  " << try_critical_path << " ";
    bool checkFlag = false;

    if (tmp_SubLoop_CriticalPath.find(tmp_SubLoop) ==
        tmp_SubLoop_CriticalPath.end())
      checkFlag = true;
    ...
    if (checkFlag) {
      ...
      //  (3a)  -- find the successors of the loop by checking its exiting
      //  blocks' successors and continue the DFS
      ...
      for (auto ExitB : tmp_SubLoop_ExitingBlocks) {
        for (auto B : successors(ExitB)) {
          if (L->contains(B) && BlockVisited.find(B) == BlockVisited.end()) {
            *Evaluating_log
                << "---- continue to traverser to Block: " << B->getName()
                << " ";
            LoopLatencyEvaluation_traversFromHeaderToExitingBlocks(
                try_critical_path, L, B);
          }
        }
      }
    }
  } else {
    //     (3b) -- If it is a block out of sub-loops, evaluate the block latency
    //     and update the critical path if necessary (max(ori_CP, lastStateCP +
    //     BlockLatency)).
    *Evaluating_log << " which is  not evaluated in Loop "
                    << " ";
    double latency_CurBlock = BlockLatencyEvaluation(
        curBlock); // first, get the latency of the current block
    ...
    *Evaluating_log << " NewCP =  " << try_critical_path << " ";
    ...
    if (checkFlag) // update the block-level critical path
    {
      ...
      for (auto B : successors(curBlock)) {
        if (L->contains(B) && BlockVisited.find(B) == BlockVisited.end()) {
          *Evaluating_log << "---- continue to traverser to Block: "
                          << B->getName() << " ";
          LoopLatencyEvaluation_traversFromHeaderToExitingBlocks(
              try_critical_path, L, B);
        }
...
/*
    Simply mark all the blocks in the loop with the totoal_latency by
   DFS-traverse
*/
void SimpleTimingEvaluation::MarkBlock_traversFromHeaderToExitingBlocks(
    double total_latency, Loop *L, BasicBlock *curBlock) {
  BlockVisited.insert(curBlock);
