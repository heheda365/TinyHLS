...
*/
double SimpleTimingEvaluation::getFunctionLatency(Function *F) {
  ...
  *Evaluating_log << "\n\n\nDone evaluation Function Latency for Function "
                  << F->getName() << " and its latency is "
                  << max_critial_path_in_F << " cycles.\n";
  *Evaluating_log << "---- Function " << F->getName() << " includes "
  ...
  for (auto tmpL : Function2OuterLoops[F]) {
    *Evaluating_log << " Loop:" << tmpL->getName()
                    << "(lat=" << LoopLatency[tmpL] << ") ";
  }
  *Evaluating_log << "\n---- Function " << F->getName()
                  << " includes following block(s) out of loops:\n-------";
  for (auto &B_it : *F) {
    if (!isInLoop(&B_it))
      *Evaluating_log << " Block:" << B_it.getName()
                      << "(lat=" << BlockLatency[&B_it] << ") ";
  }
  *Evaluating_log << "\n\n\n\n";
...
/*
    traverse the block in function by DFS to find the longest paths to each
   block in the function: (1) Mark the block visited, as a step of typical DFS
    (2) Check whether the search reaches a block in the loops
    (3a) -- If it is a block in loops, regard the most outer loop as
   intergration and update the critical path if necessary (max(ori_CP,
   lastStateCP + LoopLatency)).
         -- find the successors of the loop by checking its exiting blocks'
   successors and continue the DFS (3b) -- If it is a block out of loops,
...
*/
void SimpleTimingEvaluation::getFunctionLatency_traverseFromEntryToExiting(
  ...
  *Evaluating_log << "---- FUNCTION traverser arrive Block: "
                  << curBlock->getName() << " ";

  // (2) Check whether the search reaches a block in the loops
  if (isInLoop(curBlock)) {
    // (3a) -- If it is a block in loops, regard the loop as intergration and
    ...
    *Evaluating_log << "---- Block: " << curBlock->getName()
    ...
    double latency_Loop =
        getOuterLoopLatency(tmp_OuterLoop); // treat the entire loop as a block
                                            // node and get the latency
    ...
    *Evaluating_log << " NewCP =  " << try_critical_path << " ";
        ...
        for (auto B : successors(ExitB)) {
          if (F == B->getParent() &&
            ...
            *Evaluating_log
                ...
                << "\n";
            getFunctionLatency_traverseFromEntryToExiting(try_critical_path, F,
                                                          B);
          }
        }
      }
    ...
    //     (3b) -- If it is a block out of loops, evaluate the block latency and
    //     update the critical path if necessary (max(ori_CP, lastStateCP +
    //     BlockLatency)).
    *Evaluating_log << "---- Block: " << curBlock->getName()
