...
bool SimpleTimingEvaluation::runOnModule(
    Module &M) // The runOnFunction declaration will overide the virtual one in
               // ModulePass, which will be executed for each Function.
{
  ...
  for (auto &F : M) {
    ...
    mangled_name = "find function " + mangled_name +
                   "and its demangled name is : " + demangled_name;
    ...
    if (demangled_name == top_function_name) {
      ...
      *Evaluating_log << "Done latency evaluation of top function: "
                      << F.getName() << " and its latency is "
                      << top_function_latency << "\n\n\n";
    }
  }
...
bool SimpleTimingEvaluation::CheckDependencyFesilility(Function &F) {
  for (auto &B : F)
    for (auto &I : B)
      if (CallInst *CI = dyn_cast<CallInst>(&I)) {
        if (FunctionLatency.find(CI->getCalledFunction()) ==
            FunctionLatency.end()) {
          return false;
        }
      }
...
char SimpleTimingEvaluation::ID =
...
void SimpleTimingEvaluation::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
