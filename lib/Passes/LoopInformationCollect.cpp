...
bool LoopInFormationCollect::runOnLoop(
    Loop *L,
    LPPassManager &) // The runOnLoop declaration will overide the virtual one
                     // in LoopPass, which will be executed for each Loop.
{
  // Loop_out->flush();
  ...
  if (L->isInvalid()) {
    ...
    errs() << "Loop-ID:" << Loop_id[L]
           << " is invalid but being processed by pass.\n";
  }
  //   else
  //  return false;

  *Loop_out << "L_ID" << Loop_id[L] << "--getSmallConstantMaxTripCount="
